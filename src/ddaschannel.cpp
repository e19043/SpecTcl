...
void ddaschannel::SetChannelID(uint  data){
  chanid = ((int ) ((data & CHANNELIDMASK)));
}

void ddaschannel::SetSlotID(uint  data){
  slotid = ((int ) ((data & SLOTIDMASK) >> 4));
}

void ddaschannel::SetCrateID(uint  data){
  crateid = ((int ) ((data & CRATEIDMASK) >> 8));
}

void ddaschannel::SetChannelHeaderLength(uint  data){
  channelheaderlength = ((int ) ((data & HEADERLENGTHMASK) >> 12 ));
}

void ddaschannel::SetChannelLength(uint  data){
  channellength = ((int ) ((data & CHANNELLENGTHMASK) >> 17));
}

void ddaschannel::SetOverflowCode(uint  data){
  overflowcode = ((int ) ((data & OVERFLOWMASK) >> 30));
}

void ddaschannel::SetFinishCode(uint  data){
  finishcode = ((int ) ((data & FINISHCODEMASK) >> 31 ));
}

void ddaschannel::SetID(uint  data){
  //broken function
}

void ddaschannel::SetTimeLow(uint  data){
  timelow = data;
}

void ddaschannel::SetTimeHigh(uint  data){
  timehigh = ((int ) ((data & LOWER16BITMASK)));
}
...
void ddaschannel::SetEnergy(uint  data){
  energy = ((int ) ((data & LOWER16BITMASK)));
}

void ddaschannel::SetTraceLength(uint  data){
  tracelength = ((int ) ((data & UPPER16BITMASK) >> 16));
}

void ddaschannel::SetModMSPS(uint  data){
  ModMSPS = ((int ) (data & LOWER16BITMASK) );
}

void ddaschannel::SetEnergySums(uint  data){
  energySums.push_back(data);
}

void ddaschannel::SetQDCSums(uint  data){
...
void ddaschannel::UnpackChannelData(TranslatorPointer<uint32_t>& data){
  ...
  *data++;
  ...
  if(channelheaderlength != channellength){
    ...
    //more unpacking data
    cout << channelheaderlength << " " << channellength << endl;
